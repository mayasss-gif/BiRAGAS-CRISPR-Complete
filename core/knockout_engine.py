...
class KnockoutEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self._mc_samples = self._config.get('mc_samples', 1000)
    ...
    def _bayes_propagate(self, dag, gene: str) -> float:
        ...
        try:
            paths = list(nx.all_simple_paths(dag, gene, [
    ...
    def _feedback_propagate(self, dag, gene: str) -> float:
        ...
        try:
            cycles = [c for c in nx.simple_cycles(dag) if gene in c and len(c) <= 5]
        except Exception:
            cycles = []
